...
parser.add_argument("--clt_path", default=dirname, help="path to the command-line-tools")
parser.add_argument(
    "--output",
    "-o",
...
clt_path = pathlib.Path(args.clt_path)
    ...
    if not args.no_mp4:
        ...
        width, height = (
            ...
            for value in subprocess.run(
                [str(clt_path / "size"), str(input_file)],
                check=True,
                capture_output=True,
            ).stdout.split(b"x")
        )
